 ...
 *      Let V be a non-empty and finite set, its elements being called 
 *      vertices (or nodes). Let E be a subset of the Cartesian product V×V, 
 ...
 *      a sink, if for every node w in G that is reachable from v, v is also 
 *      reachable from w. The bottom of a graph is the subset of all nodes 
 *      that are sinks, i.e., bottom(G)={v∈V∣∀w∈V:(v→w)⇒(w→v)}. You have to 
 *      calculate the bottom of certain graphs.
 *  
 *  Input Specification
 *      The input contains several test cases, each of which corresponds to 
 *      a directed graph G. Each test case starts with an integer number v, 
 *      denoting the number of vertices of G=(V,E) where the vertices will be 
 *      identified by the integer numbers in the set V={1,…,v}. You may assume 
 *      that 1≤v≤5000. That is followed by a non-negative integer e and, 
 *      thereafter, e pairs of vertex identifiers v1,w1,…,ve,we with the 
 *      meaning that (vi,wi)∈E. There are no edges other than specified by 
...
void dfsRev(vector<int>* edgesT, int start, unordered_set<int>& visited, unordered_set<int>* component){
    visited.insert(start);
    component->insert(start);
    for(int i=0; i<edgesT[start].size(); i++){
        int adjacent = edgesT[start][i];
        if(visited.count(adjacent) == 0){
            dfsRev(edgesT, adjacent, visited, component);
        }
    }
}
...
unordered_set<unordered_set<int>*>* getSCC(vector<int>* edges, vector<int>* edgesT, int n){
    ...
    while(!finishedVertices.empty()){
        ...
        if(visited.count(element) == 0){
            ...
            dfsRev(edgesT, element, visited, component);
...
int main() {
    while(true){
        ...
        // check if the SCC are bottom or not
