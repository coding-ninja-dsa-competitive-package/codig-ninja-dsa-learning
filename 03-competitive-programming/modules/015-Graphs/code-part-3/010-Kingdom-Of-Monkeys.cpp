 ...
 *      rituals are performed. Each ritual teams up two monkeys. If Monkeys 
 *      A and B teamed up and Monkeys B and C teamed up, then Monkeys A and C 
 *      are also in the same team.
 *  -   You are given an array A where Ai is the number of bananas i'th monkey 
 *      gathers.
 *  -   Find out the number of gold coins that our Monk should set aside for 
 *      the prize.
 *  
 *  Input:
 *      First line contains an integer T. T test cases follow. First line of 
 *      each test case contains two space-separated N and M. M lines follow. 
 *      Each of the M lines contains two integers Xi and Yi, the indexes of 
 *      monkeys that perform the i'th ritual. Last line of the testcase 
 *      contains N space-separated integer constituting the array A.
 *  Output:     
...
ll numOfBananas(vector<ll>* edges, ll* arr, ll n, bool* visited, ll start){
    ll countBananas = arr[start];
    visited[start] = true;
    for(ll i=0; i<edges[start].size(); i++){
        ll adjacent = edges[start][i];
        if(!visited[adjacent]){
            countBananas += numOfBananas(edges, arr, n, visited, adjacent);
        }
    }
    return countBananas;
}
...
int main() {
    ...
    while(t--){
        ...
        for(ll i=0; i<n; i++){
            if(!visited[i]){
                ll numBananas = numOfBananas(edges, arr, n, visited, i);
                if(numBananas > maximum){
                    maximum = numBananas;
                }
            }
        }
