 ...
 *      such that no two edges intersect and all angles are less than 180 
 *      degrees. We can represent a polygon by listing all the vertices, 
 *      starting at one vertex and following the edges until that vertex is 
 *      reached again. Thus, element 0 in the array represents the first 
 *      vertex. The first vertex is connected to the second vertex (element 1), 
 *      the second vertex is connected to the third vertex (element 2) and so 
 *      on. The last element represents the last vertex, which is connected to 
 *      the first vertex.
 *  -   Given the vertices of a polygon, where the x-coordinate of vertex i is 
 *      element i of int[] x and its y-coordinate is element i of int[] y, 
